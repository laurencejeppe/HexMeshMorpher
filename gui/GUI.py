...
class AmbergThread(QThread):
    ...
    def run(self):
        AM = AmbergMapping(sourcey=self.source,
                                          targety=self.target,
                                          mappedy=self.output,
                                          steps=self.steps,
...
class RBF_Morpher(QMainWindow):
    ...
    def handle_result(self, result:Mesh):
...
if __name__=="__main__":
    ...
    try:
        with open(
                "asa/styles/styles.css", "r", encoding="utf-8"
                ) as file:
            app.setStyleSheet(file.read())
    except FileNotFoundError:
        print("No style file found, using default style.")
    win.show()
    sys.exit(app.exec())
